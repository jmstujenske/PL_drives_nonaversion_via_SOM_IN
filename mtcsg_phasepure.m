function [yo, fo, to]=mtcsg_phasepure(varargin);
%function [yo, fo, to]=mtcsg(x,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers);
% Multitaper Time-Frequency Cross-Spectrum, only considering phase
...
% Modified by Joseph M. Stujenske in 2020, to only include phase information
...
for Ch1 = 1:nChannels
	for Ch2 = Ch1:nChannels % don't compute cross-spectra twice
		...
		Temp2 = conj(Temp2);
        Temp1 = Temp1./abs(Temp1);%set amplitude to 1 to eliminate amplitude information
        Temp2 = Temp2./abs(Temp2);%set amplitude to 1 to eliminate amplitude information
		Temp3 = (Temp1) .* (Temp2);
		eJ=sum(Temp3, 2);
...
%
% Originally written by Kenneth D. Harris 
