function [yo, fo, to]=mtcsg_parallel(varargin)
%function [yo, fo, to]=mtcsg_parallel(x,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers);
% Multitaper Time-Frequency Cross-Spectrum (cross spectrogram)
% function A=mtcsg(x,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers)
% x : input time series
...
% Modified by Joseph M. Stujenske in 2020 to use the parallel computing
% toolbox to speed up computations
...
Periodogram = complex(zeros(nFreqBins, nTapers, nFFTChunks, nChannels)); % intermediate FFTs
...
% 		Temp2 = reshape(Periodogram(:,:,:,Ch2), [nFreqBins,nTapers,nFFTChunks]);
		Temp2 = conj(Periodogram(:,:,:,Ch2));
		Temp3 = Periodogram(:,:,:,Ch1) .* Temp2;
		eJ=sum(Temp3, 2);
